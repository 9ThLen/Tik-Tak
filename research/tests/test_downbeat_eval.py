...
def estimate_from(ref, beats_per_bar=None, phase=0, margin=1.0, strength=1.0,
                  shift=0.0):
...
def test_a_perfect_answer_scores_perfectly():
    ...
    assert result.verdict(0.25) == Verdict.CORRECT
...
def test_the_bar_line_on_the_wrong_beat_is_a_phase_error():
    ...
    assert result.verdict(0.25) == Verdict.WRONG_PHASE
...
def test_calling_four_four_two_four_is_caught_even_though_the_f_measure_looks_fine():
    ...
    assert result.verdict(0.25) == Verdict.WRONG_METER
...
def test_finding_nothing_is_not_the_same_as_being_wrong():
    ...
    nothing = Estimate(beats=ref.beats, downbeats=np.zeros(0), beats_per_bar=0,
                       downbeat_strength=0.0, downbeat_margin=0.0)
    result = score(ref, nothing)

    assert result.meter_correct is None
    assert result.verdict(0.25) == Verdict.NO_ANSWER
    assert result.verdict(0.0) == Verdict.NO_ANSWER
...
def test_a_low_margin_withholds_rather_than_answers_wrong():
    ...
    assert score(ref, wrong).verdict(0.25) == Verdict.WITHHELD
    assert score(ref, wrong).verdict(0.05) == Verdict.WRONG_PHASE
...
def test_unscorable_clips_are_left_out_of_the_sweep():
    ...
    rows = sweep([good, tapped], thresholds=[0.0])
    assert rows[0].n == 1
...
def test_raising_the_threshold_trades_coverage_for_safety():
    ...
    low, high = sweep(scores, thresholds=[0.0, 0.5])
...
def test_the_counts_account_for_every_clip():
    ...
    for row in sweep(scores, thresholds=[0.0, 0.2, 0.5, 1.5]):
...
def test_the_threshold_is_the_most_generous_one_inside_the_budget():
    ...
    rows = sweep(scores, thresholds=[0.0, 0.5, 0.95])
    chosen = choose_margin(rows, max_wrong_rate=0.0)

    assert chosen is not None
    assert chosen.min_margin == pytest.approx(0.5)
...
def test_no_acceptable_threshold_is_an_answer_and_not_a_crash():
    ...
    assert choose_margin(sweep(scores, thresholds=[0.0, 0.5, 1.5]), 0.05) is None
...
def test_a_threshold_that_never_fires_is_not_a_calibration():
    ...
    rows = sweep(scores, thresholds=[0.9])
    assert rows[0].shown == 0
    assert choose_margin(rows, max_wrong_rate=0.0) is None


def test_choosing_on_an_empty_split_returns_nothing():
    assert choose_margin(sweep([], thresholds=[0.0, 0.5]), 0.05) is None


def test_precision_can_be_bought_with_coverage_which_is_why_both_are_reported():
    scores = _scores([(1.0, 0, 4)] + [(0.2, 2, 4)] * 5)
    rows = {r.min_margin: r for r in sweep(scores, thresholds=[0.0, 0.5])}
