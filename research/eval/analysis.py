...
class Estimate:
    ...
    live_tempo_spreads_octaves: np.ndarray = field(
        default_factory=lambda: np.zeros(0, dtype=np.float64))
    # The activation-tempo estimator's own answer, beside the filter's. Kept
    # separately because only their product reaches the beat list, and a run at
    # the wrong metrical level cannot otherwise say which of the two chose it.
    live_anchor_bpm: np.ndarray = field(
        default_factory=lambda: np.zeros(0, dtype=np.float64))
    live_anchor_confidence: np.ndarray = field(
        default_factory=lambda: np.zeros(0, dtype=np.float64))
    live_anchor_margin: np.ndarray = field(
        default_factory=lambda: np.zeros(0, dtype=np.float64))
    ...
    def from_json(cls, payload: dict) -> "Estimate":
        return cls(
            ...
            live_anchor_bpm=np.asarray(
                payload.get("live_anchor_bpm", []), dtype=np.float64),
            live_anchor_confidence=np.asarray(
                payload.get("live_anchor_confidence", []), dtype=np.float64),
            live_anchor_margin=np.asarray(
                payload.get("live_anchor_margin", []), dtype=np.float64),
...
class Analyser:
    ...
    def analyse_live_file(
        ...
        *,
        ...
        ``seeded`` hands the tracker the tempo an offline analysis of the same
        file found, before a sample is played. That is not a shippable mode —
