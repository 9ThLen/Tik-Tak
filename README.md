...
семплово-точний метроном і настільний стенд для перевірки затримки та джитера.
...
| Синхронне відтворення імпортованої мінусовки | 🚧 Наступна фаза |
| Онлайн-трекер із мікрофона | ⏳ Заплановано |
| Автоматичне визначення сильної долі та розміру | ⏳ Заплановано |
...
7. **Кешування.** Сітка серіалізується в переносний бінарний блок. Ключем є
...
8. **Планування підказок.** Планувальник видає події наперед і окремо компенсує
...
У цьому режимі аудіо має надходити невеликими блоками без зупинки. Потоковий
ODF уже реалізований, але онлайн-трекер, який повинен адаптувати темп і фазу до
живого виконання, ще не входить до `main`. Запланований трекер також має
ігнорувати власний клік метронома, щоб застосунок не синхронізувався сам із
собою.

### Manual + sync

Користувач задає BPM вручну, а застосунок має знайти музичну атаку й вирівняти
фазу метронома з початком фрази. Ядро вже вміє змінювати темп і прив'язувати
сітку до заданого часу, але автоматичне визначення моменту синхронізації ще
заплановане.

### Сильна доля та розмір

Метроном уміє відтворювати різні звуки для сильної долі, звичайної долі та
субподілів, якщо розмір задано вручну. Автоматичне визначення початку такту й
розмірів 4/4, 3/4 або 6/8 потребуватиме окремої моделі та ще не реалізоване.

## Режими

| Режим | Що отримує користувач | Поточний стан |
|---|---|---|
| **Файл** | Аналіз мінусовки, BPM, впевненість і точні часові мітки долей | Аналіз готовий; синхронне відтворення треку ще розробляється |
| **Auto** | Метроном слідує за живим гуртом або інструментом через мікрофон | Заплановано |
| **Manual + sync** | BPM задається вручну, а фаза вирівнюється з музикою | Частково підтримано ядром; повний сценарій заплановано |
...
- C++-тести перевіряють FFT, mel-фільтри, STFT, ODF, темп, трекер, декодер,
  кеш, C API, планувальник і семплово-точний клік.
